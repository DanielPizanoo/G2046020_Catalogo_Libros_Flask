...
app = Flask(__name__)
db = SQLAlchemy(app)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql+psycopg2://postgres:12345@localhost:5432/libros_db'
...
def registrarAutor():
    ...
    autor_nuevo = Autor(nombre_autor=nombre_autor,
...
def registrarLibro():
    ...
    libro_nuevo = Libro(titulo_libro=titulo_libro, fecha_publicacion=fecha_publicacion, numero_paginas=numero_paginas,
                        formato=formato, id_editorial=id_editorial, id_genero=id_genero, id_autor=id_autor)
...
def consultaLibro():
    ...
    dict_consultas = {}
    # Consultas a la base de datos
    consulta_libros = Libro.query.join(Genero, Libro.id_genero == Genero.id_genero).join(Autor, Libro.id_autor == Autor.id_autor).join(Editorial,
                                                                                                                                       Libro.id_editorial == Editorial.id_editorial).add_columns(Libro.id_libro, Libro.titulo_libro, Libro.fecha_publicacion,
                                                                                                                                                                                                 Libro.numero_paginas, Libro.formato, Editorial.nombre_editorial, Genero.nombre_genero, Autor.nombre_autor)
    # Convertir en diccionario
...
def consultaFavorito():
    ...
    dict_consultas = {}
    # Consultas a la base de datos
    consulta_favorito = Libro.query.join(Usuarios, Favorito.id_usuario == Usuarios.id).join(Libro,
                                                                                            Favorito.id_libro == Libro.id_libro).add_columns(Favorito.id_favorito, Usuarios.email, Libro.titulo_libro)
